...
bool Modeling::findCircleFunc()
{
	...
	for (int i = 0; i < circle_list.size(); i++) {
		...
		Point2f find_cent = minEllipse.center;
		cout << find_cent << endl;
		find_cent.x += info.ROI.x;
		find_cent.y += info.ROI.y;
		std::sprintf(text, "x:%.1f ,y:%.1f", find_cent.x, find_cent.y);
