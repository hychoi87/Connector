...
public:
	...
	bool LoadModeingINI();
	bool loadModeingINIByField();
	bool loadModeingINIByFieldSecond();
	...
	void diff_img(Mat input, Mat &result, int location);
	//쎌인포 읽어서 좌표 이동 시켜줄 예정정정정
	bool readCellInfo();
	void parseModelingInfo(string line, ModelingInfo *ptemp);
	//사출물 전용
	bool findCircleFunc(const int ID);
	bool findLineFunc(const int ID);
	//란삭
	bool ransacEllipseFitting(Mat test, vector<Point> &input_point, int N, double dist_th, int max_iter, int num_th);
	bool ransacLineFitting(Mat test, vector<Point> &input_point, int N, double dist_th, int max_iter, int num_th);
...
private:
	...
	vector <ModelingInfo> CENTER_List;
	vector <ModelingInfo> HOUGH_List;
	vector <ModelingInfo> circle_list;
	vector <ModelingInfo> line_list;
	vector <ModelingInfo> ID_list;
